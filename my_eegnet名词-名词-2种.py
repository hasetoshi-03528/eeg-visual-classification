    ...
    for step, (b_x, b_y) in enumerate(dataloader):
        ...
        if step % 5 == 0:
          with torch.no_grad():
            ...
            # train_out, train_last_layer = eeg_net(train_x.to(DEVICE))
            # pred_train_y = torch.max(train_out.cpu(), 1)[1].data.numpy()
            pred_y = torch.max(test_out.cpu(), 1)[1].data.numpy()
            # train_accuracy = float((pred_train_y == train_y.data.numpy()).astype(int).sum()) / float(train_y.size(0))
            test_accuracy = float((pred_y == test_y.data.numpy()).astype(int).sum()) / float(test_y.size(0))
            # print('Epoch: ', epoch, '| train loss: %.4f' % loss.cpu().data.numpy(),'| train accuracy: %.3f' % train_accuracy, '| test accuracy: %.3f' % test_accuracy)
